...
// Overall Parameters
	...
	ratchetAdjust=-15; 		// needs to be adjusted based on number of spokes (e.g. 20 for 5 spokes)
...
module backFrame()
{
	...
	translate(
		[(showToPrint==true ? axis_separation/2 : 0),
		0,
		(showAssembly==true ? -addFrameThickness-0.5-1*deltaZ-2*explodeZ : 0)])
	union()
	placeWheel(fold_angle,axis_separation,0)
	{
		rotate(180,[0,0,1])
